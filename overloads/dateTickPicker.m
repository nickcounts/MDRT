...
% Add a scaling factor to pad the date tick labels (add additional space)
scalePercent = 1.1;
...
%---------------------------------------------------------------%
function extent = localGetExtentHG2(formlen,format,ax,axh,dateInd,axPos)
...
%---------------------------------------------------------------%
function extent = localGetExtentHG1(formlen,format,ax,axh,dateInd,axPos)
...
if ax == 0 %0 is the X-axis
    ...
    extent = ext(3)/max(formlen)*len/axPos(3);
else
    set(htemp,'string',('WW')')
    ext = get(htemp,'extent');
    % fractional height taken up by string
    extent = repmat(ext(4),size(format))/axPos(4);
end
delete(tempfig)
...
% If the date form contains year information:spaceport
